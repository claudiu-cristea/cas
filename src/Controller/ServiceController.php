...
class ServiceController implements ContainerInjectionInterface {
  ...
  /**
   * The CAS URL helper service.
   *
   * @var \Drupal\cas\Service\CasUrlHelper
   */
  protected $casUrlHelper;
   ...
   * @param \Drupal\cas\Service\CasUrlHelper $cas_url_helper
   *   The CAS URL helper service.
   */
  public function __construct(CasHelper $cas_helper, CasValidator $cas_validator, CasUserManager $cas_user_manager, CasLogout $cas_logout, RequestStack $request_stack, UrlGeneratorInterface $url_generator, ConfigFactoryInterface $config_factory, MessengerInterface $messenger, EventDispatcherInterface $event_dispatcher, ExternalAuthInterface $external_auth, PrivateTempStoreFactory $private_tempstore_factory, CasUrlHelper $cas_url_helper) {
    ...
    $this->privateTempStore = $private_tempstore_factory->get('cas');
    $this->casUrlHelper = $cas_url_helper;
  }
  ...
  public function handle() {
    ...
    if (!$request->query->has('ticket')) {
      ...
      $this->casUrlHelper->handleReturnToParameter($request);
    ...
    // @see \Drupal\cas\Event\CasRedirectEvent
    $cas_redirect_event = new CasPreUserLoadRedirectEvent($ticket, $cas_validation_info);
    $this->casHelper->log(LogLevel::DEBUG, 'Dispatching EVENT_REDIRECT.');
    $this->eventDispatcher->dispatch(CasHelper::EVENT_PRE_USER_LOAD_REDIRECT, $cas_redirect_event);

    // A subscriber might have set an HTTP redirect response allowing potential
    // user interaction to be injected into the flow.
    if ($redirect_response = $cas_redirect_event->getRedirectResponse()) {
      // Make available relevant data to the target page but store it into the
      // private temp store instead of passing it to the redirect URL.
      $this->privateTempStore->set('cas_context', [
        'ticket' => $ticket,
        'property_bag' => $cas_validation_info,
   ...
   *
   * @deprecated in cas:8.x-1.6 and will be removed from cas:8.x-1.10. Use the
   *   ::handleReturnToParameter() method of 'cas.url_helper' service instead.
   */
  private function handleReturnToParameter(Request $request) {
    @trigger_error('\Drupal\cas\Controller\ServiceController::handleReturnToParameter() in cas:8.x-1.6 and will be removed from cas:8.x-1.10. Use \Drupal::service("cas.url_helper")->handleReturnToParameter().', E_USER_DEPRECATED);
    $this->casUrlHelper->handleReturnToParameter($request);
  }
